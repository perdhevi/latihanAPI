...
docker compose --profile backup run --rm --no-deps --entrypoint age-keygen backup >"$work/key.txt" 2>/dev/null
recipient=$(grep -o 'age1[0-9a-z]*' "$work/key.txt")
...
file=$(ls -t backups/latihan-*.dump.age | head -1)
...
docker compose --profile backup run --rm -v "$work/key.txt:/identity:ro" backup restore.sh "/backups/$(basename "$file")"
